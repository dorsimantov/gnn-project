...
dataset.data.iso_type_2 = F.one_hot(dataset.data.iso_type_2, num_classes=num_i_2).to(
    torch.float
)

dataset.data.iso_type_3 = torch.unique(dataset.data.iso_type_3, True, True)[1]
num_i_3 = dataset.data.iso_type_3.max().item() + 1
dataset.data.iso_type_3 = F.one_hot(dataset.data.iso_type_3, num_classes=num_i_3).to(
    torch.float
)
...
for i in range(SPLITS):
    ...
    learning_indices = [
        x for idx, x in enumerate(range(n * i, n * (i + 1))) if x % 4 <= 1
    ]
...
for epoch in range(EPOCHS):
    print_or_log(
        "Epoch " + str(epoch + 1) + ":" + str(mean_tst_accuracies[epoch]),
        log_file_path="log"
        + MODEL
        + DATASET
        + ","
